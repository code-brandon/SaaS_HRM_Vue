          ...
          <div style=" width: 3000px;">
            <table border="0" align="center" cellpadding="0" cellspacing="0" class="tableInfo">
              ...
              <tr v-for="(item, index) in baseData" :key="item.id">
                ...
                <td
                  v-for="(it,ind) in item.attendances"
...
export default {
  ...
  methods: {
    ...
    setupUI() {
      ...
      this.formData.data.day = moment(this.localDate).format('YYYY-MM-DD')
    ...
    async getdepartment() {
      await list().then(data => {
        this.departmentData = data.data.data.departments
      })
    },
    // 初始化数据
    async dataList(dataParams) {
      ...
      await attendancesList(dataParams).then(data => {
        ...
        if (data.page.list === null) {
          ...
          this.baseData = data.page.list
          this.attendInfo.counts = data.page.totalCount
          var year = this.localDate.getFullYear()
          console.log(this.formData.data.day)
          var month = moment(this.formData.data.day).format('MM')
          this.attendInfo.month = month
    ...
    handleOver(item, id, it) {
      ...
      this.modifyData.userId = item.userId
      this.modifyData.mobile = item.mobile
      this.modifyData.username = item.username
