...
import { resignation, resignationDetail } from '@/api/base/employees'
...
export default {
  ...
  methods: {
    ...
    getObjInfo() {
      ...
      resignationDetail({ id: this.$route.params.id }).then(ret => {
    ...
    submitData(obj) {
      this.$refs[obj].validate(valid => {
        if (valid) {
          this.formBase.userId = this.$route.params.id
          resignation(this.formBase).then(() => {
            this.$router.push({
