...
export default {
  ...
  methods: {
    async init() {
      ...
      let opType = 1
      const { data: listRes } = await getArchivingCont({ yearsMonth: yearMonth })
      console.log(listRes)
      this.contentData = listRes.data.page.list
    },
    handleExport() {
    ...
    async archive() {
      let yearsMonth = this.yearMonth
      const { data: archiveRes } = await getArchivingArchive({ yearsMonth })
    },
    async createNewReport(yearMonth) {
      const { data: archiveRes } = await newReport({ dataMonth: yearMonth })
    },
    // 归档报表
