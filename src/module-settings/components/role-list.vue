...
export default {
  ...
  methods: {
    assignPrem() {
      assignPrem({roleId:this.formData.id,permIds:this.$refs.tree.getCheckedKeys()}).then(res => {
    ...
    handlerPerm(obj) {
       detail({id:obj.id}).then(res=>{
         ...
         this.checkNodes = res.data.data.permIds != null ? res.data.data.permIds.split(','):[]
          permApi.list({type:0,pid:null,enVisible:1}).then(res => {
            this.treeData = commonApi.transformTozTreeFormat(res.data.data.page.list)
    ...
    doQuery() {
      list(this.requestParameters).then(res => {
          ...
          this.counts = res.data.data.page.totalPage
        })
    },
    // 每页显示信息条数
